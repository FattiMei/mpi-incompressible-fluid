...
// this test solves the 1D problem laplacian(p) = ... with periodic boundary conditions with a serial algorithm
int main(int argc, char *argv[]) {
	...
	// cos(x) for x in [0, 2*pi] solves the problem
	const Real h = 2.0*M_PI/(N);
	for (int i = 0; i < N; ++i) {
		xex[i] = std::cos(h*i);
		b[i]   = -xex[i];
	}
