...
namespace mif {
    ...
    inline Real compute_Y2_u(const VelocityTensor &U, Real t_n, size_t i, size_t j, size_t k) {
        COMPUTE_Y2(u, calculate_momentum_rhs_with_forcing_u);
    }
    inline Real compute_Y2_v(const VelocityTensor &U, Real t_n, size_t i, size_t j, size_t k) {
        COMPUTE_Y2(v, calculate_momentum_rhs_with_forcing_v);
    }
    inline Real compute_Y2_w(const VelocityTensor &U, Real t_n, size_t i, size_t j, size_t k) {
    ...
    #define COMPUTE_Y3(component, equation)                                                                       \
        const Real dt = U.constants.dt;                                                                           \
        return U.component(i,j,k) + dt * (a31 * equation(U, i, j, k, t_n) + a32 * equation(Y2, i, j, k, time_1)); \
        
    inline Real compute_Y3_u(const VelocityTensor &U, const VelocityTensor &Y2, Real t_n, Real time_1, size_t i, size_t j, size_t k) {
        COMPUTE_Y3(u, calculate_momentum_rhs_with_forcing_u);
    }
    inline Real compute_Y3_v(const VelocityTensor &U, const VelocityTensor &Y2, Real t_n, Real time_1, size_t i, size_t j, size_t k) {
        COMPUTE_Y3(v, calculate_momentum_rhs_with_forcing_v);
    }
    inline Real compute_Y3_w(const VelocityTensor &U, const VelocityTensor &Y2, Real t_n, Real time_1, size_t i, size_t j, size_t k) {
    ...
    #define COMPUTE_U_STAR(component, equation)                                                                 \
        const Real dt = U.constants.dt;                                                                         \
        return U.component(i,j,k) + dt * (b1 * equation(U, i, j, k, t_n) + b3 * equation(Y3, i, j, k, time_2)); \
        
    inline Real compute_u_star_u(const VelocityTensor &U, const VelocityTensor &Y3, Real t_n, Real time_2, size_t i, size_t j, size_t k) {
        COMPUTE_U_STAR(u, calculate_momentum_rhs_with_forcing_u);
    }
    inline Real compute_u_star_v(const VelocityTensor &U, const VelocityTensor &Y3, Real t_n, Real time_2, size_t i, size_t j, size_t k) {
        COMPUTE_U_STAR(v, calculate_momentum_rhs_with_forcing_v);
    }
    inline Real compute_u_star_w(const VelocityTensor &U, const VelocityTensor &Y3, Real t_n, Real time_2, size_t i, size_t j, size_t k) {
