...
void C2Decomp::writeOne(int ipencil, double *var, string filename) {
  ...
  MPI_File_write_all(fh, var, subsizes[0] * subsizes[1] * subsizes[2],
                     data_type, MPI_STATUS_IGNORE);
...
void C2Decomp::readOne(int ipencil, double *var, string filename) {
  ...
  MPI_File_read_all(fh, var, subsizes[0] * subsizes[1] * subsizes[2], data_type,
                    MPI_STATUS_IGNORE);
...
void C2Decomp::writePlane(int ipencil, double *var, int iplane, int n,
  ...
  if (iplane == 0) {
    ...
    for (int kp = 0; kp < decompMain.xsz[2]; kp++) {
      for (int jp = 0; jp < decompMain.xsz[1]; jp++) {
        ...
        int ii3 = kp * decompMain.xsz[1] * decompMain.xsz[0] +
                  jp * decompMain.xsz[0] + n;
        wk2d[ii2] = wk[ii3];
      }
    }
    ...
    for (int kp = 0; kp < decompMain.ysz[2]; kp++) {
      for (int ip = 0; ip < decompMain.ysz[0]; ip++) {
        ...
        int ii3 = kp * decompMain.ysz[1] * decompMain.ysz[0] +
                  n * decompMain.ysz[0] + ip;
        wk2d[ii2] = wk[ii3];
      }
    }
    ...
    for (int jp = 0; jp < decompMain.zsz[1]; jp++) {
      for (int ip = 0; ip < decompMain.zsz[0]; ip++) {
        ...
        int ii3 = n * decompMain.zsz[1] * decompMain.zsz[0] +
                  jp * decompMain.zsz[0] + ip;
        wk2d[ii2] = wk[ii3];
      }
    }
  ...
  disp = 0;
  MPI_File_set_view(fh, disp, data_type, new_type, "native", MPI_INFO_NULL);
  MPI_File_write_all(fh, wk2d, subsizes[0] * subsizes[1] * subsizes[2],
                     data_type, MPI_STATUS_IGNORE);
