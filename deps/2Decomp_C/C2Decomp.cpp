...
void C2Decomp::decomp2DInit(int pRow, int pCol) {
  ...
  ierr = MPI_Cart_create(MPI_COMM_WORLD, 2, dims, periodic, 0,
                         &DECOMP_2D_COMM_CART_X);
...
void C2Decomp::decompInfoInit() {
  ...
  if (nx % dims[0] == 0 && ny % dims[0] == 0 && ny % dims[1] == 0 &&
      nz % dims[1] == 0) {
...
void C2Decomp::prepareBuffer(DecompInfo *dii) {
  ...
  dii->x1count =
      dii->x1dist[dims[0] - 1] * dii->y1dist[dims[0] - 1] * dii->xsz[2];
  dii->y1count = dii->x1count;

  dii->y2count =
      dii->y2dist[dims[1] - 1] * dii->z2dist[dims[1] - 1] * dii->zsz[0];
