...
int main(int argc, char *argv[]) {
  ...
  if (!mpiRank)
    cout << "Initializing random field... " << endl;
  ...
  if (!mpiRank)
    cout << "Calculating Divergence in xpencil... " << endl;
  ...
  if (!mpiRank)
    cout << "Calculating Divergence in ypencil... " << endl;
  ...
  if (!mpiRank)
    cout << "Calculating Divergence in zpencil... " << endl;
  ...
  if (!mpiRank)
    cout << "Calculating Divergence using halo's in xpencil" << endl;
  for (int kp = 0; kp < xSize[2]; kp++) {
    for (int jp = 0; jp < xSize[1]; jp++) {
      for (int ip = 1; ip < xSize[0] - 1; ip++) {
        int ii = kp * xSize[1] * xSize[0] + jp * xSize[0] + ip;
  ...
  for (int kp = 0; kp < xSize[2]; kp++) {
    for (int jp = 0; jp < xSize[1]; jp++) {
      for (int ip = 1; ip < xSize[0] - 1; ip++) {
        ...
        // some macros or functions to retrieve this information for a given
        // pencil would be nice...
        int iihp1 = (kp + numLevel) * (xSize[1] + 2 * numLevel) * xSize[0] +
                    (jp + numLevel + 1) * xSize[0] + ip;
        int iihm1 = (kp + numLevel) * (xSize[1] + 2 * numLevel) * xSize[0] +
                    (jp + numLevel - 1) * xSize[0] + ip;
  ...
  for (int kp = 0; kp < xSize[2]; kp++) {
    for (int jp = 0; jp < xSize[1]; jp++) {
      for (int ip = 1; ip < xSize[0] - 1; ip++) {
        ...
        // some macros or functions to retrieve this information for a given
        // pencil would be nice...
        int iihp1 = (kp + numLevel + 1) * (xSize[1] + 2 * numLevel) * xSize[0] +
                    (jp + numLevel) * xSize[0] + ip;
        int iihm1 = (kp + numLevel - 1) * (xSize[1] + 2 * numLevel) * xSize[0] +
                    (jp + numLevel) * xSize[0] + ip;
  ...
  MPI_Allreduce(&errorFlag, &errorFlagGlobal, 1, MPI_C_BOOL, MPI_LOR,
                MPI_COMM_WORLD);
  ...
  if (!mpiRank)
    cout << "Calculating Divergence using halo's in ypencil" << endl;
  ...
  for (int kp = 0; kp < ySize[2]; kp++) {
    for (int jp = 1; jp < ySize[1] - 1; jp++) {
      for (int ip = 0; ip < ySize[0]; ip++) {
        ...
        int iihp1 = (kp + numLevel) * ySize[1] * (ySize[0] + 2 * numLevel) +
                    jp * (ySize[0] + 2 * numLevel) + ip + numLevel + 1;
        int iihm1 = (kp + numLevel) * ySize[1] * (ySize[0] + 2 * numLevel) +
                    jp * (ySize[0] + 2 * numLevel) + ip + numLevel - 1;
  ...
  for (int kp = 0; kp < ySize[2]; kp++) {
    for (int jp = 1; jp < ySize[1] - 1; jp++) {
      for (int ip = 0; ip < ySize[0]; ip++) {
        ...
        int iihp1 = (kp + numLevel + 1) * ySize[1] * (ySize[0] + 2 * numLevel) +
                    jp * (ySize[0] + 2 * numLevel) + ip + numLevel;
        int iihm1 = (kp + numLevel - 1) * ySize[1] * (ySize[0] + 2 * numLevel) +
                    jp * (ySize[0] + 2 * numLevel) + ip + numLevel;
  ...
  if (!mpiRank)
    cout << "Calculating Divergence using halo's in zpencil" << endl;
  ...
  for (int kp = 1; kp < zSize[2] - 1; kp++) {
    for (int jp = 0; jp < zSize[1]; jp++) {
      for (int ip = 0; ip < zSize[0]; ip++) {
        ...
        int iihp1 =
            (kp) * (zSize[1] + 2 * numLevel) * (zSize[0] + 2 * numLevel) +
            (jp + numLevel) * (zSize[0] + 2 * numLevel) + ip + numLevel + 1;
        int iihm1 =
            (kp) * (zSize[1] + 2 * numLevel) * (zSize[0] + 2 * numLevel) +
            (jp + numLevel) * (zSize[0] + 2 * numLevel) + ip + numLevel - 1;
  ...
  for (int kp = 1; kp < zSize[2] - 1; kp++) {
    for (int jp = 0; jp < zSize[1]; jp++) {
      for (int ip = 0; ip < zSize[0]; ip++) {
        ...
        int iihp1 =
            (kp) * (zSize[1] + 2 * numLevel) * (zSize[0] + 2 * numLevel) +
            (jp + numLevel + 1) * (zSize[0] + 2 * numLevel) + ip + numLevel;
        int iihm1 =
            (kp) * (zSize[1] + 2 * numLevel) * (zSize[0] + 2 * numLevel) +
            (jp + numLevel - 1) * (zSize[0] + 2 * numLevel) + ip + numLevel;
