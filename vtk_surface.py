...
if __name__ == "__main__":
    ...
    points = np.array(
        struct.unpack(
            ">" + 3 * npoints * typechar, data[point_data_start:point_data_end]
        )
    )
    ...
    yz_face = remove_duplicate_points(
        np.array(sorted(points[points[:, 0] == closest_x].tolist()))
    )
    xz_face = remove_duplicate_points(
        np.array(sorted(points[points[:, 1] == closest_y].tolist()))
    )
    xy_face = remove_duplicate_points(
    ...
    byte_cells = bytearray().join(
        struct.pack(">iiiii", int(c[0]), int(c[1]), int(c[2]), int(c[3]), int(c[4]))
        for c in cells
    )

    cell_types = bytearray().join(struct.pack(">i", 9) for _ in range(len(cells)))
